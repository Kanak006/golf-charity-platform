...
import {algorithmicDraw, calculatePrizePools, checkMatch, randomDraw} from '@/lib/draw-engine'
...
export async function POST(req: NextRequest) {
  ...
  const {drawType = 'random', simulate = false, drawDate} =
      await req.json() as {
    drawType: DrawType
    simulate: boolean
    drawDate: string
  ...
  const {data: activeSubscriptions} =
      await admin.from('subscriptions').select('user_id').eq('status', 'active')

  const userIds = activeSubscriptions?.map(s => s.user_id) ||
      []

      // Get scores for eligible users
      const {data: allScores} = await admin.from('scores').select('*').in(
                                    'user_id', userIds)

  // Generate winning numbers
  const drawResult = drawType === 'algorithmic' ?
      algorithmicDraw(allScores || []) :
      randomDraw()

  // Calculate prize pools
  const {data: subscriptionData} = await admin.from('subscriptions')
                                       .select('amount_cents')
                                       .eq('status', 'active')
  ...
  const scoresByUser: Record<string, number[]> = {} allScores?.forEach(s => {
    if (!scoresByUser[s.user_id]) scoresByUser[s.user_id] = []
      scoresByUser[s.user_id].push(s.score)
  })

  // Check each user
  const match5Winners: string[] = [] const match4Winners: string[] =
      [] const match3Winners: string[] = []

  Object.entries(scoresByUser).forEach(([userId, userScores]) => {
    const {matchType, matchedNumbers} =
        checkMatch(userScores, drawResult.winning_numbers)
    ...
    if (matchType) {
      ...
      winnerMap[userId] = { matchType, matched: matchedNumbers, prize }
    }
  })

  if (simulate) {
    // Return simulation data without saving
  ...
  if (error || !draw) return NextResponse
      .json({error: 'Failed to create draw'}, {status: 500})

      // Save prize pools
      await admin.from('prize_pools')
      .insert([
        {
          draw_id: draw.id,
          pool_type: 'jackpot_5',
  ...
  const winnerInserts = Object.entries(winnerMap).map(
      ([userId, data]) => ({
        ...
        payment_status: data.matchType === 'match_5' ? 'verification_required' :
                                                       'pending',
      }))

  if (winnerInserts.length > 0) await admin.from('winners').insert(
      winnerInserts)

  return NextResponse.json({draw, winner_count: winnerInserts.length})
}